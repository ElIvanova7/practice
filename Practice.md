...
Пользователь создает заказ при помощи корзины, из каталога товары добавляются в нее. Здесь же пользователь может выбрать адрес доставки: по умолчанию будет стоять адрес, указанный пользователем в его личном профиле (если указан) или же можно ввести любой другой, соответствующий радиусу доставки. Также в корзине отображается товар с краткой информацией для удобства пользователя, предусмотрена функция удаления из корзины. Указана полная стоимость заказа: "Итого к оплате:". Для оформления заказа необходимо нажать на кнопку "Оформить заказ", после чего пользователь будет перенаправлен на страницу оплаты. 
